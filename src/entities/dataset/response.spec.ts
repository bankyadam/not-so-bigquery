...
import DatasetResponse from './response';
...
describe('DatasetResponse', function() {
  it('extends BaseEntityResponseObject', function() {
    expect(new DatasetResponse('p', 'd')).to.be.instanceof(BaseEntityResponse);
  });

  it('has proper TYPE set', function() {
    expect(new DatasetResponse('p', 'd')).to.have.property('TYPE').that.eql('dataset');
  });
