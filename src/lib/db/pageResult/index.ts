...
export default async function(query, queryData, maxResults = MAX_RESULTS, pageToken?, queryId?): Promise<PageResult> {
  const cache = new QueryCache();
  maxResults = maxResults < 1 ? MAX_RESULTS : Math.min(maxResults, MAX_RESULTS);
  ...
  const totalRows = await cache.getTotalRows();
