...
import { BIGQUERY_TYPES, BIGQUERY_DATE_PARTS, BIGQUERY_DATE_TYPES } from '../../bigQuery/types';
import * as FUNCTION_HANDLERS from '../functions';
...
export default (parser) => {
  ...
  class Visitor extends BaseCstVisitorWithDefaults {
    ...
    _anyWord(token) {
      if (RESERVED_WORDS.includes(token.image.toUpperCase())) {
        return token.image;
      }
      return `"${token.image}"`;
    }
