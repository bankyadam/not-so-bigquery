...
describe('Datasets', function() {
  after(async function() {
    await commands.cleanupDatasets(config.DATASET_NAME);
  });

  it('checks for a dataset that does not exist', async function() {
    const [realShouldNotExists, fakeShouldNotExists] =
      await commands.runEverywhere(async conn => (await conn.dataset(config.DATASET_NAME).exists())[0]);
  ...
  it('creates dataset', async function() {
    const [realCreateDatasetResult, fakeCreateDatasetResult] =
      await commands.runEverywhere(async conn => conn.createDataset(config.DATASET_NAME));
  ...
  it('deletes an existing dataset', async function() {
    const [realDeleteDatasetResult, fakeDeleteDatasetResult] =
      await commands.runEverywhere(async conn => conn.dataset(config.DATASET_NAME).delete({ force: true }));
